...
# Extract ENV_ environment variables and build the sed expression
env | grep '^ENV_' | while IFS='=' read -r key value; do
...
if [ ! -s "$sed_script" ]; then
    echo "No ENV_ environment variables found. Exiting with error."
    exit 1
fi

# sed All files
find /usr/share/nginx/html -type f -exec sed -i -f "$sed_script" '{}' +s
