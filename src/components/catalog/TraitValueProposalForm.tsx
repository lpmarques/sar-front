...
export default function TraitValueProposalForm({
  ...
  const validateTraitValue = (value: TraitValue) => {
    ...
    switch (trait.type) {
      case "string":
        ...
        return textOptions.includes(str) ? null : 'Item inválido';
      case "string[]":
  ...
  return (
    ...
    <Paper withBorder style={{ backgroundColor: "#f0f2f2" }} ta="center" p={15} mb={20}>
      ...
      {divider}
      <Button type="submit" color="teal" onClick={handleSubmit} loading={proposalCreation.isPending}>Publicar proposta</Button>
    </Paper>
    </>
  )
}
